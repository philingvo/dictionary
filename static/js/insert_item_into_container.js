...
function fillDroplist(dropListContainer, containerItems, itemName) {
	...
	containerItems.forEach(function(item) {
		// option = getOptionElement(item[itemName].id, `${item.position}. ${item[itemName].title}`);
...
function changeSubject(event) {
	...
	if (subject_id != 'null') {
		// console.log(`http://${location.host}/api/topics/?subject_id=${subject_id}`);
...
function changeTopic(event) {
	...
	if (topic_id != 'null') {
		// console.log(`http://${location.host}/api/sets_by_topics_id/?topics_id=${topic_id}`);
		downloadJson(`http://${location.host}/api/sets_by_topics_id/?topics_id=${topic_id}`, fillSets);
		// window.setsList.classList.remove('hidden');
	} else {
		window.setsList.classList.add('hidden');
		hideFinalElements();
	};
...
//http://127.0.0.1:8050/api/set_with_elements/?set_id=1
//http://127.0.0.1:8050/insert_element_into_set/1/